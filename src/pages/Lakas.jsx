...
import {
  getWeeklyScore, FOOD_PRESETS, RECOMP_STARTER_RULES, RECOMP_NUTRITION_GUIDE,
  BEGINNER_PHASES, FORM_GUIDES, GYM_SESSION_TYPES, EXERCISE_VIDEO_GUIDES,
  BUILT_IN_ROUTINES, MEAL_TYPES, GOAL_TYPES, ROUTINE_FOCUS, ACTIVITY_TYPES,
  REMINDER_TYPES, REMINDER_FREQUENCIES, WEEK_DAYS, ROUTINE_LIBRARY_META,
  ...
  buildTemplateNotes, getExerciseVideoGuide, safeScrollIntoView, getYouTubeEmbedUrl,
  getPreferredTrainingTrack, getBeginnerTemplateCycle, getResolvedRoutineDuration,
  getExerciseLoadHint, getExerciseAlternativeHint, calculatePlates, getPlateStyle,
  GENERAL_EXERCISE_SWAPS, getExerciseSwapsList, getExerciseActiveSeconds,
  estimateExerciseMinutes, estimateRoutineMinutes, getExerciseSetCount,
  getCompletedSetCount, buildCompletedSetState, formatDurationClock,
  normalizeRows, normalizeExerciseKey, normalizeExerciseSecondaryMuscles,
  sanitizeExerciseLibrary, mergeExerciseLibrary, buildExerciseMetaMap,
  dateDaysAgo, sortNewest, revokeObjectUrl, sortOldest, numberOrZero,
  roundToNearest, formatNumber, formatMonthLabel, getReferenceBodyWeight,
  getRecompProteinGoal, getRecompStarterDefaults, normalizePrimaryGoal,
  getNutritionMode, getRecoveryState, getWeightTrendSummary,
  getTemplateForSessionKey, getGoalDefaultSessionKey, getGoalLabel,
  estimateOneRepMax, hydrateExerciseRows, sanitizeExerciseRows,
  getExerciseTotals, formatExerciseLine, buildExerciseHistoryMap,
  buildExerciseAnalytics, buildMonthlyReport, buildYearInReview,
  formatPreviousExerciseSummary, getRoutineGroupLabel, groupRoutinesByLabel,
  displayMetric, calculateBmi, getBmiLabel, getLastDateKeys, addMonths,
  getMonthDays, createDateMap, calculateWorkoutStreak, getPersonalRecords,
  getBeginnerProgression, getHabitScore,
        ...
        <>
	        <section className={`${lStyles.panel} ${lStyles.workoutLaunchPad}`}>
	          ...
	          <div className={lStyles.workoutHero}>
                {gymSessionMode.startedAt && !gymSessionMode.open && (
                  <article className={`${lStyles.workoutLaunchCard} ${lStyles.workoutPrimaryCard}`} style={{ backgroundColor: 'var(--blue)', borderColor: 'var(--blue)', marginBottom: '16px' }}>
                    <span style={{ color: 'var(--blue-900)' }}>Workout in progress</span>
                    <strong style={{ color: 'var(--blue-900)' }}>Resume {gymSessionMode.templateName || 'Session'}</strong>
                    <small style={{ color: 'var(--blue-800)' }}>You have an active workout in progress. Elapsed: {formatDurationClock(activeGymElapsedSeconds)}.</small>
                    <div className={lStyles.workoutLaunchActions} style={{ marginTop: '12px' }}>
                      <Button type="button" variant="primary" fullWidth onClick={() => setGymSessionMode(current => ({ ...current, open: true }))} style={{ backgroundColor: 'var(--blue-900)', color: 'white' }}>
                        Resume Logging
                      </Button>
                    </div>
                  </article>
